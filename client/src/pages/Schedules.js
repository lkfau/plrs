...
function Schedules() {
    ...
    const getSchedules = async () => {
        const response = await fetch(`${process.env.REACT_APP_SERVER_URL}/schedules?user_id=1&get_items=true`);
    ...
    const confirmDeleteModalHandler = async (confirm) => {
        if (confirm && scheduleToDelete != null) {
            const response = await fetch(`${process.env.REACT_APP_SERVER_URL}/schedules?schedule_id=${scheduleToDelete.schedule_id}`, {
                method: 'DELETE',
                headers: {
